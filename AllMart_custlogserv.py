...
def login(master):
    master.destroy()
    f = ('Times', 14)
    con=myconn.connect(host='localhost',user='root',password="@Ayush4321",database="ALLMART")
    cur = con.cursor()
    cur.execute('CREATE TABLE IF NOT EXISTS record(NAME VARCHAR(25) , EMAIL VARCHAR(20) , CONTACT_NUMBER VARCHAR(10), GENDER VARCHAR(10) , state VARCHAR(20),PASSWORD VARCHAR(10) )')
    con.commit()
    wst=tk.Tk()
    wst.geometry("1530x800")
    wst.title("ALLMART")
    wst.configure(background='lightgoldenrod1')
    fr1=tk.Frame(wst)
    fr1.pack(side="top")
    ...
    b_title=tk.Message(text=" AllMart -:Login ",relief="raised",width=4900,padx=700,pady=0,fg="darkolivegreen1",bg="gray1",justify="center",anchor="center")
    b_title.config(font=("Bahnschrift Light SemiCondensed","28",))
    b_title.pack(side="top")
    b_title=tk.Message(text="Please Login to continue with various services of  Allmart ",relief="raised",width=1000,padx=100,pady=10,fg="darkolivegreen1",bg="gray1",justify="left",anchor="e")
    b_title.config(font=("Bahnschrift Light SemiCondensed","18",))
    b_title.pack(side="top")
    img2=(Image.open("logi.jpg"))
    reimg2=img2.resize((420,425),Image.ANTIALIAS)
    new2=ImageTk.PhotoImage(reimg2)
    img_label2=tk.Label(wst,image=new2)
    img_label2.place(x=15,y=150)
    img3=(Image.open("logi2.png"))
    reimg3=img3.resize((420,425),Image.ANTIALIAS)
    new3=ImageTk.PhotoImage(reimg3)
    img_label3=tk.Label(wst,image=new3)
    img_label3.place(x=870,y=150)
    ...
    def login_response():
        ...
        var.set('male')
# widgets
    left_frame = Frame(wst,bd=2,bg='#CCCCCC',relief=SOLID,padx=20,pady=20)
    ...
    register_btn = Button(left_frame,width=15,text='Register',font=f,relief=SOLID,cursor='hand2',command=lambda: register(wst))
# widgets placement
    email_tf.grid(row=0, column=1, pady=10, padx=20)
    pwd_tf.grid(row=1, column=1, pady=10, padx=20)
    login_btn.grid(row=2, column=1, pady=10, padx=20)
    register_btn.grid(row=3, column=1, pady=10, padx=20)
    left_frame.place(x=450, y=150)   
    img7=tk.PhotoImage(file="back.png")
    myimg7=img7.subsample(1,1)
    b7=tk.Button(image=myimg7,command=lambda:a(wst))   
    b7.image=myimg7
    b7.place(x=475,y=420)
# infinite loop
    wst.mainloop()
def a(master):
    master.destroy()
...
def billing(master):
    ...
    class bill_app:
        ...
        #subclothing
            ...
            self.price_polo=400,500,600
            self.price_peter=700,900,1300
            self.price_zodiac=2000,2500

            self.tshirt=["NIKE","PUMA","H&M"]
            self.price_nike=450,550
            self.price_puma=720,900
            self.price_hm=850,1300
...
#code to be corrected from here....
        #image2
            img_1=Image.open("p2.jpg")
        ...
        #image3
            ...
            lbl_title=Label(self.root,text="OWN BILL MAKER OF ALLMART",font=("arial",30,"bold"),bg="black",fg="yellow")
        ...
        #image 4 chatbot
            bg_img=Image.open("image/superbotlogo.jpg")
            bg_img=bg_img.resize((860,200),Image.BOX)
            self.bg_image=ImageTk.PhotoImage(bg_img)
        
            lbl_bg_img=Button(main_frame,image=self.bg_image,command=self.chatbot)
            lbl_bg_img.place(x=0,y=121,width=860,height=200)

            B1=Button(main_frame,text="Chatbot",command=self.chatbot,font=("times new roman",15,"bold"),bg="black",fg="white",width=15,cursor="hand2")
            B1.place(x=10,y=320,height=40,width=200)
        ...
        #BUTTON
            ...
            self.BtnAddToCart2=Button(button_frame,command=self.iprint,text="Send Bill",font=("times new roman",15,"bold"),bg="black",fg="white",width=15,cursor="hand2")
    ...
    #To print
        def iprint(self):
            ...
            mydb=myconn.connect(host="localhost",user="root",password="@Ayush4321",database="pending")
            mycursor=mydb.cursor()

            dat=json.dumps(c)
            q="INSERT INTO bill (orderid,items) VALUES (%s,%s)"
            data=(y,dat)

            mycursor.execute(q,data)
    
            mydb.commit()
            messagebox.showinfo("Message","Your bill is sent to the user...")
    #To search
...
def register(master):
    ...
    con.commit()          
    ws=tk.Tk()
    ws.geometry("1530x800")
    ws.title("ALLMART")
    ws.configure(background='lightgoldenrod1')
    fr1=tk.Frame(ws)
    fr1.pack(side="top")
    l_title=tk.Message(text="Welcome To AllMart ",relief="raised",width=4900,padx=700,pady=0,fg="darkolivegreen1",bg="gray1",justify="center",anchor="center")
    l_title.config(font=("Bahnschrift Light SemiCondensed","28",))
    l_title.pack(side="top")
    b_title=tk.Message(text=" AllMart -:Registration ",relief="raised",width=4900,padx=700,pady=0,fg="darkolivegreen1",bg="gray1",justify="center",anchor="center")
    b_title.config(font=("Bahnschrift Light SemiCondensed","28",))
    b_title.pack(side="top")
    b_title=tk.Message(text="Join the Family Of Allmart ",relief="raised",width=1000,padx=100,pady=10,fg="darkolivegreen1",bg="gray1",justify="left",anchor="e")
    b_title.config(font=("Bahnschrift Light SemiCondensed","18",))
    b_title.pack(side="top")
    img2=(Image.open("register2.jpg"))
    reimg2=img2.resize((420,425),Image.ANTIALIAS)
    new2=ImageTk.PhotoImage(reimg2)
    img_label2=tk.Label(ws,image=new2)
    img_label2.place(x=15,y=150)
    img3=(Image.open("register3.jpg"))
    reimg3=img3.resize((420,425),Image.ANTIALIAS)
    new3=ImageTk.PhotoImage(reimg3)
    img_label3=tk.Label(ws,image=new3)
    img_label3.place(x=900,y=150)
    def insert_record():
        check_counter=0
        warn = ""
        ...
        if check_counter == 8:        
            try:
                ...
                a="INSERT INTO record (NAME,EMAIL,CONTACT_NUMBER,GENDER,COUNTRY,PASSWORD) VALUES('{}','{}','{}','{}','{}','{}')".format(register_name.get(),register_email.get(),register_mobile.get(),var.get(),register_state,register_pwd.get())
                cur.execute(a)
            
                con.commit()
                x=messagebox.showinfo('confirmation', 'Record Saved')
                if x=="ok":
                    #ws.destroy()
                    login(ws)
...
# The Whole Page is been divided into 2 parts using frames This is the description of the right frame  ws is the main window,bd border,bg background,relief for special effects.
    right_frame = Frame(ws,bd=2,bg='#CCCCCC',relief=SOLID,padx=20,pady=20)
# It helps in providing a space to write the details in the page. grid is used to set the geometry of the box
    ...
    Label(right_frame,text="Select state", bg='#CCCCCC',font=f).grid(row=4, column=0, sticky=W, pady=10)
...
# Enter is used to take input from the user as the user writes the input in tgose spaces
    ...
    var = StringVar()
    var.set('male')
# Radiobutton is used to provide multiple choice question only one can choice , variable means the value will change so it is set var means string variable , so if you want to enter the integer type so in that case IntVar() is used
#value isused to identify which radio button is selected.
...
#states is an empty list in which names would be appended 
    ...
    for state in world:
        state = state.rstrip('\n')
        states.append(state)
    variable.set(states[22])

    register_state = OptionMenu(right_frame,variable,*states)

    register_state.config(width=15,font=('Times', 12))
...
# Button is used to trigger some activity 
    ...
    register_state.grid(row=4, column=1, pady=10, padx=20)
    register_pwd.grid(row=5, column=1, pady=10, padx=20)
    pwd_again.grid(row=6, column=1, pady=10, padx=20)
    register_btn.grid(row=7, column=1, pady=10, padx=20)
    right_frame.place(x=450, y=150)
    ...
    img7=tk.PhotoImage(file="back.png")
    myimg7=img7.subsample(1,1)
    b7=tk.Button(image=myimg7,command=lambda:a(ws))   
    b7.image=myimg7
    b7.place(x=475,y=635)
...
def main(name):
    ...
    rootwn.title("ALL MART -:Customer Inteface")
    ...
    label.pack(side="top")
    label=tk.Label(text="Logged in as -: "+name,relief="raised",bg="gray1",font=("Times",16),fg="darkolivegreen1",anchor="center",justify="center")
    label.pack(side="top")
    def upd():
    ...
    label.pack(side="top")
    upd()
    img1=(Image.open("t1.jpeg"))
    reimg1=img1.resize((450,185),Image.ANTIALIAS)
    new1=ImageTk.PhotoImage(reimg1)
    img_label1=tk.Label(rootwn,image=new1)
    img_label1.place(x=810,y=130)
    img3=(Image.open("st.jpeg"))
    reimg3=img3.resize((450,185),Image.ANTIALIAS)
    new3=ImageTk.PhotoImage(reimg3)
    img_label3=tk.Label(rootwn,image=new3)
    img_label3.place(x=810,y=350)


    img2=tk.PhotoImage(file="billing1.png")
    myimg2=img2.subsample(1,1)
    img3=tk.PhotoImage(file="review.png")
    myimg3=img3.subsample(1,1)
    img5=tk.PhotoImage(file="changeacc.png") #account change details button #cart button
    myimg5=img5.subsample(1,1)
    b2=tk.Button(image=myimg2,command=lambda: billing(rootwn))
    b2.image=myimg2
    b3=tk.Button(image=myimg3,command=lambda:review(rootwn))
    b3.image=myimg3
    #b4=tk.Button(image=myimg4,command=lambda: disp_bal(accnt))
    #b4.image=myimg4
    b5=tk.Button(image=myimg5,command=lambda: changeacc(rootwn))
    b5.image=myimg5
...
def review(master):
    ...
    root = tk.Tk()
    mess= Label(root,text="WRITE YOUR REVIEW!!!",anchor='center')
    mess.pack()
    e = Entry(root, width=100, borderwidth=10)
    mess1= Label(root,text="Please Rate Us Out Of 5 Stars!!!!!!!!!",anchor='center')
    mess1.pack()
    f=Entry(root,width=10,borderwidth=10)
    e.pack()
    f.pack()

#Connecting database to python code
    conn = myconn.connect(host='localhost',user='root',password='@Ayush4321',database='reviews',)
    ...
    def insertdata():
        review = e.get()
        rating = f.get()
        query="Insert into rev values('{}',{})".format(review,rating)
        db_cursor.execute(query)
        conn.commit()
        response = messagebox.showinfo('Continue Shopping','Thank You For The Review')
...
#Creating button for submitting and canceling 
    ...
    buttonext = Button(root, text='Cancel', fg='red',command=lambda: h(root))
...
def changeacc(master):
    ...
    l_title.config(font=("Bahnschrift Light SemiCondensed","28",))
    l_title.pack(side="top")
    b_title=tk.Message(text=" AllMart -:Data Updation ",relief="raised",width=4900,padx=700,pady=0,fg="darkolivegreen1",bg="gray1",justify="center",anchor="center")
    b_title.config(font=("Bahnschrift Light SemiCondensed","28",))
    b_title.pack(side="top")
...
# The Whole Page is been divided into 2 parts using frames This is the description of the right frame  ws is the main window,bd border,bg background,relief for special effects.
    right_frame = Frame(ws,bd=2,bg='#CCCCCC',relief=SOLID,padx=20,pady=20)
# It helps in providing a space to write the details in the page. grid is used to set the geometry of the box
    Label(right_frame,text="Which Details To Be Updated (Except Email-ID)???", bg='#CCCCCC',font=f).grid(row=0, column=0, sticky=W, pady=10)

    Label(right_frame, text="Enter Updated Detail-:", bg='#CCCCCC',font=f).grid(row=2, column=0, sticky=W, pady=10)
    Label(right_frame, text="Enter The Email-ID", bg='#CCCCCC',font=f).grid(row=1, column=0, sticky=W, pady=10)

# Enter is used to take input from the user as the user writes the input in tgose spaces

    register_detail = Entry(right_frame, font=f)

    register_email = Entry(right_frame, font=f)

    register_which=Entry(right_frame,font=f)
...
#states is an empty list in which names would be appended 
    ...
    def insert_record():
        check_counter=0
        warn = ""
        if register_detail.get() == "":
            warn = "Detail can't be empty"
        else:
            check_counter += 1
        ...
        if check_counter == 2:        
            try:
                ...
                cur = con.cursor()
                print(type(register_which))
                x=register_which.get()
                print(type(x))
                q="update record set "+x+"=\""+register_detail.get()+"\"where ""email=\""+register_email.get()+"\";"
                print(q)
                cur.execute(q)
...
# Button is used to trigger some activity 
    ...
    register_detail.grid(row=2, column=1, pady=10, padx=20)
    register_email.grid(row=1, column=1, pady=10, padx=20) 
    register_which.grid(row=0,column=1,padx=20,pady=10)
    register_btn.grid(row=7, column=1, pady=10, padx=20)
    right_frame.place(x=450, y=150)

    img7=tk.PhotoImage(file="back.png")
    myimg7=img7.subsample(1,1)
    b7=tk.Button(image=myimg7,command=lambda: h(ws))   
    b7.image=myimg7
    b7.place(x=475,y=400)
...
def mart():         
    ...
    l_title.config(font=("Bahnschrift Light SemiCondensed","28",))
    l_title.pack(side="top")
    b_title=tk.Message(text="Join the Family Of Allmart ",relief="raised",width=4900,padx=700,pady=0,fg="darkolivegreen1",bg="gray1",justify="center",anchor="center")
    b_title.config(font=("Bahnschrift Light SemiCondensed","28",))
    b_title.pack(side="top")

    b_title=tk.Message(text=" Please select one of our services to continue-:",relief="raised",width=1000,padx=100,pady=10,fg="darkolivegreen1",bg="gray1",justify="left",anchor="e")
    b_title.config(font=("Bahnschrift Light SemiCondensed","18",))
    b_title.pack(side="top")
    def upd():
        str1=strftime('%H:%M:%S %p')
    ...
    label.pack(side="top")
    upd()
    img2=(Image.open("big.jpeg"))
    reimg2=img2.resize((420,425),Image.ANTIALIAS)
    new2=ImageTk.PhotoImage(reimg2)
    img_label2=tk.Label(ws,image=new2)
    img_label2.place(x=15,y=150)
    img3=(Image.open("ui.jpeg"))
    reimg3=img3.resize((420,425),Image.ANTIALIAS)
    new3=ImageTk.PhotoImage(reimg3)
    img_label3=tk.Label(ws,image=new3)
    img_label3.place(x=900,y=150)

    img2=tk.PhotoImage(file="login.png")
    myimg2=img2.subsample(1,1)
    img3=tk.PhotoImage(file="register1.png")
    myimg3=img3.subsample(1,1)
    img5=tk.PhotoImage(file="exit.png")
    myimg5=img5.subsample(1,1)
    b2=tk.Button(image=myimg2,command=lambda: login(ws))
    b2.image=myimg2
    b3=tk.Button(image=myimg3,command=lambda:register(ws))
    b3.image=myimg3
    b5=tk.Button(image=myimg5,command=lambda: logout(ws))
    b5.image=myimg5
    b2.place(x=475,y=200)
    b3.place(x=475,y=300)
    b5.place(x=475,y=400)
    ws.mainloop()
#-------------------------------------------------------------------------------------------------------------------------------------------------------------------
mart()